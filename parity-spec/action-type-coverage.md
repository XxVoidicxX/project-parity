...
| `AUTO_MODERATION_RULE_CREATE`, `AUTO_MODERATION_RULE_UPDATE`, `AUTO_MODERATION_RULE_DELETE`, `AUTO_MODERATION_BLOCK_MESSAGE`, `AUTO_MODERATION_FLAG_TO_CHANNEL`, `AUTO_MODERATION_USER_COMMUNICATION_DISABLED` | Gateway | Exact target where Discord supplies one; action-only partial diagnosis otherwise |
...
| `VOICE_CHANNEL_STATUS_CREATE`, `VOICE_CHANNEL_STATUS_UPDATE`, `VOICE_CHANNEL_STATUS_DELETE` | Gateway | Exact target: channel ID |
| `GUILD_SOUNDBOARD_SOUND_CREATE`, `GUILD_SOUNDBOARD_SOUND_UPDATE`, `GUILD_SOUNDBOARD_SOUND_DELETE`, `GUILD_SOUNDBOARD_SOUNDS_UPDATE` | Gateway | Exact target when supplied |
| `GUILD_EXPRESSION_CREATE`, `GUILD_EXPRESSION_UPDATE`, `GUILD_EXPRESSION_DELETE` | Gateway | Exact target: expression ID |
...
Gateway delivery requires the `GUILD_MODERATION` / audit-log gateway capability exposed by the Discord library and the bot permission `VIEW_AUDIT_LOG`. REST fallback is a recovery mechanism, not a substitute for live detection.
