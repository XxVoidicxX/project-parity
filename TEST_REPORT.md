...
The local suite contains **106 tests** across three deliberately different categories. A category is not merely a language label: it exercises a different failure surface.
...
| Other (contract, cross-language, release) | 31 | shared schema, byte parity, documentation, packaging, repository hygiene |
...
| `tests/release-readiness.test.mjs` | 22 | Report/README linkage, command and release parity, CI/release workflow requirements, release versions, schemas, fixtures, action-map uniqueness, setup guidance, Components V2 live-alert detection, ignored credentials/runtime files |
...
This runs the 31 other tests, 40 JavaScript tests, and 35 Python tests. Python’s test runner emits occasional asyncio slow-task diagnostics on this machine during large concurrency cases; the suite still exits successfully and treats them as performance diagnostics, not assertions.
