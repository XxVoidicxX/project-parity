...
The local suite contains **101 tests** across three deliberately different categories. A category is not merely a language label: it exercises a different failure surface.
...
| Other (contract, cross-language, release) | 26 | shared schema, byte parity, documentation, packaging, repository hygiene |
...
| `tests/release-readiness.test.mjs` | 17 | Report/README linkage, release versions, schemas, fixtures, action-map uniqueness, setup guidance, live harness configuration, ignored credentials/runtime files |
...
This runs the 26 other tests, 40 JavaScript tests, and 35 Python tests. Python’s test runner emits occasional asyncio slow-task diagnostics on this machine during large concurrency cases; the suite still exits successfully and treats them as performance diagnostics, not assertions.
