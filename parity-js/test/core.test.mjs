...
test('all representative coverage actions reconcile exactly', async () => { const actions = ['GUILD_UPDATE','CHANNEL_DELETE','CHANNEL_OVERWRITE_UPDATE','MEMBER_KICK','MEMBER_BAN_ADD','ROLE_UPDATE','INVITE_DELETE','WEBHOOK_CREATE','EMOJI_DELETE','STICKER_UPDATE','INTEGRATION_DELETE','STAGE_INSTANCE_CREATE','GUILD_SCHEDULED_EVENT_UPDATE','THREAD_DELETE','APPLICATION_COMMAND_PERMISSION_UPDATE','AUTO_MODERATION_RULE_CREATE','AUTO_MODERATION_QUARANTINE_USER','ONBOARDING_UPDATE','VOICE_CHANNEL_STATUS_UPDATE','SOUNDBOARD_SOUND_CREATE','MESSAGE_PIN']; for (const actionType of actions) { const ledger = new Ledger({ clock }); await ledger.record(intent({ actionType, correlationId: actionType })); assert.equal(await new Reconciler({ ledger, clock }).reconcile(event({ actionType })), null, actionType); } });
